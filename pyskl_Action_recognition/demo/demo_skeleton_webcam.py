...
def parse_args():
    ...
    parser.add_argument('video', nargs='?', help='video file/url')
    parser.add_argument('out_filename', nargs='?', help='output filename')
    parser.add_argument(
        '--config',
    ...
    if not args.webcam and (args.video is None or args.out_filename is None):
        parser.error("the following arguments are required: video, out_filename")
...
def webcam_inference(args):
    """Perform inference on webcam input."""
    cap = cv2.VideoCapture(0)  # 0번 카메라를 사용
    if not cap.isOpened():
    ...
    while True:
        ...
        # 사람 감지
        det_result = inference_detector(model, frame)
        det_result = det_result[0][det_result[0][:, 4] >= args.det_score_thr]

        # 포즈 추정
        pose_result = inference_top_down_pose_model(pose_model, frame, [dict(bbox=x) for x in list(det_result)], format='xyxy')[0]

        # 포즈 시각화
        vis_frame = vis_pose_result(pose_model, frame, pose_result)

        # 결과 출력
        cv2.imshow('Webcam Inference', vis_frame)
