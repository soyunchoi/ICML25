...
class Printer:
    ...
    def draw(self, figures, axes, image, dic_out, pifpaf_out=None):
        ...
        if self.zz_pred is not None:
            ...
            self._front_loop(iterator, axes, number, colors_front, pifpaf_out, dic_out)
            ...
            # legend 크기와 위치 조정
            legend = axes[1].legend(loc='upper right',          # 오른쪽 상단에 고정
                                  fontsize=8,                   # 폰트 크기 줄임
                                  bbox_to_anchor=(1, 1),        # 위치 고정
                                  bbox_transform=axes[1].transAxes,  # 축 기준 상대 위치
                                  framealpha=0.9)               # 배경 투명도
            legend.set_draggable(False)                        # 드래그 불가능하게 설정
